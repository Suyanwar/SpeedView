...
/**
 * Created by Suyanwar on 2019-12-18.
 * Android Engineer
 */
class CustomPointerSpeedometer @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : Speedometer(context, attrs, defStyleAttr) {
    private val markPath = Path()
    private val speedometerPaint = Paint(Paint.ANTI_ALIAS_FLAG)
    private val pointerPaint = Paint(Paint.ANTI_ALIAS_FLAG)
    private val pointerWhitePaint = Paint(Paint.ANTI_ALIAS_FLAG)
    ...
    private fun initAttributeValue() {
        ...
        pointerWhitePaint.color =whiteColor
    }
    override fun onSizeChanged(w: Int, h: Int, oldW: Int, oldH: Int) {
        super.onSizeChanged(w, h, oldW, oldH)
        val risk = getSpeedometerWidth() * .5f + dpTOpx(16f) + padding.toFloat()
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (withPointer) {
            ...
            canvas.drawCircle(size * .5f, getSpeedometerWidth() * .5f + dpTOpx(16f) + padding.toFloat(), getSpeedometerWidth() * .5f + dpTOpx(10f), pointerBackPaint)
            canvas.drawCircle(size * .5f, getSpeedometerWidth() * .5f + dpTOpx(16f) + padding.toFloat(), getSpeedometerWidth() * .5f + dpTOpx(5f), pointerWhitePaint)
            canvas.drawCircle(size * .5f, getSpeedometerWidth() * .5f + dpTOpx(16f) + padding.toFloat(), getSpeedometerWidth() * .5f + dpTOpx(1f), pointerPaint)
            canvas.restore()
        }
    ...
    private fun updateRadial() {
        val centerColor = Color.argb(180, Color.red(pointerColor), Color.green(pointerColor), Color.blue(pointerColor))
        val edgeColor = Color.argb(10, Color.red(pointerColor), Color.green(pointerColor), Color.blue(pointerColor))
        val pointerGradient = RadialGradient(size * .5f, getSpeedometerWidth() * .5f + dpTOpx(16f) + padding.toFloat(), getSpeedometerWidth() * .5f + dpTOpx(10f), intArrayOf(centerColor, edgeColor), floatArrayOf(.4f, 1f), Shader.TileMode.CLAMP)
